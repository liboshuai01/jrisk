package cn.liboshuai.jrisk.module.infra.service.codegen.inner;
...
import cn.liboshuai.jrisk.framework.apilog.core.enums.OperateTypeEnum;
...
import cn.liboshuai.jrisk.framework.common.util.date.LocalDateTimeUtils;
import cn.liboshuai.jrisk.framework.common.util.object.BeanUtils;
import cn.liboshuai.jrisk.framework.common.util.object.ObjectUtils;
import cn.liboshuai.jrisk.framework.common.util.string.StrUtils;
...
import cn.liboshuai.jrisk.module.infra.dal.dataobject.codegen.CodegenColumnDO;
import cn.liboshuai.jrisk.module.infra.dal.dataobject.codegen.CodegenTableDO;
import cn.liboshuai.jrisk.module.infra.enums.codegen.CodegenFrontTypeEnum;
import cn.liboshuai.jrisk.module.infra.enums.codegen.CodegenSceneEnum;
import cn.liboshuai.jrisk.module.infra.enums.codegen.CodegenTemplateTypeEnum;
import cn.liboshuai.jrisk.module.infra.enums.codegen.CodegenVOTypeEnum;
import cn.liboshuai.jrisk.module.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 jrisk-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 jrisk-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("cn.liboshuai.jrisk.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/jrisk-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/jrisk-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "jrisk-module-${table.moduleName}/" + // 顶级模块
                "jrisk-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "jrisk-module-${table.moduleName}/" + // 顶级模块
                "jrisk-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "jrisk-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "jrisk-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "jrisk-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
