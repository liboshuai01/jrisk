package cn.liboshuai.jrisk.framework.websocket.core.sender.rabbitmq;
...
@RabbitListener(
        bindings = @QueueBinding(
                value = @Queue(
                        ...
                        name = "${jrisk.websocket.sender-rabbitmq.queue}" + "-" + "#{T(java.util.UUID).randomUUID()}",
                        // Consumer 关闭时，该队列就可以被自动删除了
                        autoDelete = "true"
                ),
                exchange = @Exchange(
                        name = "${jrisk.websocket.sender-rabbitmq.exchange}",
